...
def Deep_PSP_model_training(training_set_name, X_train_positive, X_train_negative, global_train_positive, global_train_negative, Y, X_val1, X_val2,Y_val,modelname,pretraining_model = None):
    ...
    for i in range(num):
        ...
        if pretraining_model == True:
            if training_set_name == 'kinase':
                model.load_weights(r'/home/wangxin1@1/project/DeepPSP-master/model_weight/weights_pretraining')
                if training_set_name == 'general':
                    model.load_weights(r'/home/wangxin1@1/project/DeepPSP-master/model_weight/general_model_STpre/weights%d'%(i+1))
        ...
        model.fit([X_train_, X_train2_],Y,validation_data=([X_val1, X_val2],Y_val),
              nb_epoch = 200,callbacks=callbacks_list, batch_size = 1024,verbose = 2,shuffle = True)
        model.summary()
        K.clear_session()
...
def Deep_PSP_model_testing(X_test, X_test_2, modelname,X_val1, X_val2,Y_val):
    ...
    # model.summary()
    print("预测准确率:",lr.score(X_predict_val, y_val))

    predict = lr.predict(X_predict_test)
    predict_probe = lr.predict_proba(X_predict_test)
    predict_pro = lr.predict_proba(X_predict_val)
    # y_score = model.predict_proba(X_test)
    # fpr, tpr, threshold = roc_curve(y_val, predict_pro[:,-1])
    # roc_auc = auc(fpr, tpr)
    roc_auc = roc_auc_score(y_val,predict_pro[:,-1])
